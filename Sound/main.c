...
void main() {
    ...
    while(1) {
        ...
        if (joypad_state) {
            ...
            // 7 (first digit from left)	Unused
            // 6-4	Sweep time(update rate) (if 0, sweeping is off) (over what time should it increase/decrease in pitch)
            // 3	Sweep Direction (1: decrease, 0: increase)
            // 2-0	Sweep RtShift amount (if 0, sweeping is off) (how much should the pitch should increase/decrease)
            // 0001 0110 is 0x16, sweet time 1, sweep direction increase, shift ammount per step 110 (6 decimal)
            NR10_REG = 0x16; 

            // chanel 1 register 1: Wave pattern duty (how long the wave is on or off) and sound length
            // Channels 1 2 and 4
